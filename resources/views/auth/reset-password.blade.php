...
@section('content')
    <div class="mb-5">
        ...
        <p class="text-sm text-gray-500 mt-2">Create a new password for your account.</p>
    </div>
    ...
    <form method="POST" action="{{ route('password.store') }}" class="space-y-6">
        ...
        <!-- Email Address -->
        <div x-data="{ email: '{{ old('email', $request->email) }}' }">
            ...
            <div class="relative">
                ...
                <input id="email"
                    ...
                    type="email" name="email" x-model="email" {{-- ⬅️ ATAU PAKAI INI --}} required autofocus
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input id="password"
                    class="block w-full pl-12 pr-4 py-3 border border-gray-300 rounded-xl leading-5 bg-gray-50 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all"
                    type="password" name="password" required autocomplete="new-password" placeholder="New Password" />
            </div>
            @error('password')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
