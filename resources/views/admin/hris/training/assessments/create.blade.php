...
@section('title', 'Tambah Penilaian Skill')

@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Tambah Penilaian Skill</h1>
                <p class="text-sm text-gray-500 mt-1">Buat penilaian kompetensi karyawan baru</p>
            </div>
        ...
        <form action="{{ route('hris.training.assessments.store') }}" method="POST"
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="assessment_type" id="assessment_type" required
                        ...
                        <option value="manager" {{ old('assessment_type') == 'manager' ? 'selected' : '' }}>Manager Assessment
                        </option>
                        <option value="peer" {{ old('assessment_type') == 'peer' ? 'selected' : '' }}>Peer Review</option>
                        <option value="training" {{ old('assessment_type') == 'training' ? 'selected' : '' }}>Post-Training
                        </option>
                        <option value="certification" {{ old('assessment_type') == 'certification' ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select name="proficiency_level" id="proficiency_level" required
                    ...
                    <option value="1" {{ old('proficiency_level') == '1' ? 'selected' : '' }}>1 - Novice</option>
                    <option value="2" {{ old('proficiency_level') == '2' ? 'selected' : '' }}>2 - Beginner</option>
                    <option value="3" {{ old('proficiency_level') == '3' ? 'selected' : '' }}>3 - Competent</option>
                    <option value="4" {{ old('proficiency_level') == '4' ? 'selected' : '' }}>4 - Proficient</option>
                    <option value="5" {{ old('proficiency_level') == '5' ? 'selected' : '' }}>5 - Expert</option>
                </select>
                @error('proficiency_level')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
            <div class="grid grid-cols-2 gap-6">
                <div>
                ...
                <div>
                    ...
                    <select name="assessed_by" id="assessed_by" class="w-full px-4 py-2.5 border rounded-xl">
                        ...
                        @foreach($employees as $employee)
                            <option value="{{ $employee->id }}" {{ old('assessed_by') == $employee->id ? 'selected' : '' }}>
                                {{ $employee->full_name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="3" class="w-full px-4 py-2.5 border rounded-xl"
                    placeholder="Catatan tambahan tentang penilaian...">{{ old('notes') }}</textarea>
            </div>
