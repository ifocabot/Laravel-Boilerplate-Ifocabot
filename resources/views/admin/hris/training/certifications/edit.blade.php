...
@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Edit Sertifikasi</h1>
                <p class="text-sm text-gray-500 mt-1">Ubah data sertifikasi</p>
            </div>
        ...
        <form action="{{ route('hris.training.certifications.update', $certification->id) }}" method="POST"
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="code" id="code" value="{{ old('code', $certification->code) }}" required
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="issuing_organization" id="issuing_organization"
                        value="{{ old('issuing_organization', $certification->issuing_organization) }}" required
                ...
                <div>
                    ...
                    <select name="level" id="level" class="w-full px-4 py-2.5 border rounded-xl">
                        ...
                        <option value="beginner" {{ old('level', $certification->level) == 'beginner' ? 'selected' : '' }}>
                            Pemula</option>
                        <option value="intermediate" {{ old('level', $certification->level) == 'intermediate' ? 'selected' : '' }}>Menengah</option>
                        <option value="advanced" {{ old('level', $certification->level) == 'advanced' ? 'selected' : '' }}>
                            Lanjutan</option>
                        <option value="expert" {{ old('level', $certification->level) == 'expert' ? 'selected' : '' }}>Ahli
                        </option>
                    </select>
                </div>
            </div>
            <div class="grid grid-cols-2 gap-6">
                <div>
                    <label for="validity_months" class="block text-sm font-medium text-gray-700 mb-2">Masa Berlaku
                        (Bulan)</label>
                    <input type="number" name="validity_months" id="validity_months"
                        value="{{ old('validity_months', $certification->validity_months) }}" min="0"
                        class="w-full px-4 py-2.5 border rounded-xl">
                </div>
                <div>
                    ...
                    <select name="skill_id" id="skill_id" class="w-full px-4 py-2.5 border rounded-xl">
                        ...
                        @foreach($skills as $skill)
                            <option value="{{ $skill->id }}" {{ old('skill_id', $certification->skill_id) == $skill->id ? 'selected' : '' }}>{{ $skill->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                    class="w-full px-4 py-2.5 border rounded-xl">{{ old('description', $certification->description) }}</textarea>
            </div>
            <div class="flex items-center">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $certification->is_active) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 rounded">
