...
@section('title', 'Edit Tipe Cuti - ' . $leaveType->name)
...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Edit Tipe Cuti</h1>
                <p class="text-sm text-gray-500 mt-1">{{ $leaveType->name }}</p>
            </div>
        </div>

        {{-- Form --}}
        <form action="{{ route('hris.leave.types.update', $leaveType->id) }}" method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="default_quota" id="default_quota" required min="0" max="365"
                        value="{{ old('default_quota', $leaveType->default_quota) }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent">{{ old('description', $leaveType->description) }}</textarea>
                </div>
